...
<div class="bg-base-300 rounded-lg p-4">
    <div class="flex items-center justify-between mb-3">
        ...
        <button wire:click="refreshJiraFilters" wire:loading.attr="disabled"
            wire:target="loadJiraFilters, refreshJiraFilters" class="btn btn-xs btn-ghost btn-circle"
            title="Filter neu laden">
            <svg wire:loading.remove wire:target="loadJiraFilters, refreshJiraFilters" class="w-4 h-4" fill="none"
                stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
            <span wire:loading wire:target="loadJiraFilters, refreshJiraFilters"
        ...
        @else
            <div class="space-y-2">
                @foreach ($jiraFilters as $filter)
                    <button wire:key="filter-{{ $filter['id'] }}" wire:click="loadFromFilter('{{ $filter['id'] }}')"
                        wire:loading.attr="disabled"
                        class="btn btn-sm btn-ghost justify-start w-full text-left gap-2 hover:bg-base-100">
                        <svg class="w-4 h-4 text-info shrink-0" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<div>
    ...
    <textarea wire:model.live.debounce.500ms="jiraInput"
        ...
        class="textarea textarea-bordered w-full h-28 text-sm font-mono"></textarea>
...
<button wire:click="loadFromInput" wire:loading.attr="disabled" class="btn btn-primary w-full gap-2"
    @if (empty(trim($jiraInput))) disabled @endif>
    <svg wire:loading.remove wire:target="loadFromInput" class="w-5 h-5" fill="none" stroke="currentColor"
        viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
