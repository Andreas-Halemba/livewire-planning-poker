...
<div>
    ...
    <div class="flex items-center justify-between mb-3 p-3 bg-base-300 rounded-lg">
        ...
        <span class="badge badge-primary">
            {{ count($selectedJiraTickets) }} /
            {{ count(array_filter($jiraTickets, fn($t) => !$t['alreadyImported'])) }} ausgewählt
        </span>
    </div>
    ...
    <div class="max-h-80 overflow-y-auto">
        <div class="space-y-2 p-1">
            @foreach ($jiraTickets as $index => $ticket)
                <label wire:key="jira-ticket-{{ $ticket['key'] }}"
                ...
                {{ in_array($ticket['key'], $selectedJiraTickets) ? 'ring-2 ring-primary' : 'ring-1 ring-transparent' }}">
                    <input type="checkbox" wire:model.live="selectedJiraTickets" value="{{ $ticket['key'] }}"
    ...
    {{-- Import Button --}}
    <button wire:click="importSelectedJiraTickets" wire:loading.attr="disabled"
        class="btn btn-success w-full mt-4 gap-2" @if (empty($selectedJiraTickets)) disabled @endif>
        <svg wire:loading.remove wire:target="importSelectedJiraTickets" class="w-5 h-5" fill="none"
            stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
        <span wire:loading.remove wire:target="importSelectedJiraTickets">{{ count($selectedJiraTickets) }} Ticket(s)
            importieren</span>
